...
func New(params driverParameters) (storagedriver.StorageDriver, error) {
	...
	client, err := hdfs.New(params.hdfsNameNode)
	if err != nil {
		log.Fatal(err)
	}
...
// original object.
func (d *driver) Move(ctx context.Context, sourcePath string, destPathstring string) error {
...
func (d *driver) Delete(ctx context.Context, path string) error {
	return d.hdfsClient.RemoveAll(d.fullPath(path))
}
